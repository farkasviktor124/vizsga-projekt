...
export const CartProvider = ({ children }) => {
  ...
  const submitOrder = async (orderDetails = {}) => {
    ...
    try {
      ...
      console.log("[RENDELÉS] Rendelés véglegesítése - Készlet már le van vonva a kosárba helyezéskor:", cartItems);
      
      // Itt küldheted el a rendelés adatait a backendnek
      const orderData = {
        ...orderDetails,
        items: cartItems,
        total: cartTotal,
      ...
      // Opcionális: rendelés mentése a backendbe
      try {
        await fetch("http://localhost:4000/api/rendelesek", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      // Kosár ürítése - de itt NEM állítjuk vissza a készletet!
      setCartItems([]);
      localStorage.removeItem("cart");
