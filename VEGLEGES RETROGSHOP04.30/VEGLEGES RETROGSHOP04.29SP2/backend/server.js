...
async function initializeDatabase() {
  try {
    ...
    // Garancia tábla
await dbRun(`CREATE TABLE IF NOT EXISTS garancia (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  nev TEXT NOT NULL,
  email TEXT NOT NULL,
  termekNev TEXT NOT NULL,
  datum TEXT NOT NULL,
...
// Bejelentések tábla
await dbRun(`CREATE TABLE IF NOT EXISTS bejelentesek (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  nev TEXT NOT NULL,
  email TEXT NOT NULL,
  tipus TEXT NOT NULL,
...
// Garancia igény mentése
app.post('/api/garancia', async (req, res) => {
  const { nev, email, termekNev, datum, problema } = req.body;
  
  if (!nev || !email || !termekNev || !datum || !problema) {
    return res.status(400).json({ error: "Minden mező kitöltése kötelező!" });
  }
  
  try {
    const result = await dbRun(
      `INSERT INTO garancia (nev, email, termekNev, datum, problema, status, created_at) 
       VALUES (?, ?, ?, ?, ?, 'függőben', datetime('now'))`,
      [nev, email, termekNev, datum, problema]
    );
    
    res.json({ success: true, message: "Garancia igény rögzítve!", id: result.lastID });
  } catch (error) {
    console.error('Hiba a garancia mentésekor:', error);
    res.status(500).json({ error: "Adatbázis hiba!" });
  }
});

// Összes garancia lekérése (adminnak)
app.get('/api/garancia', async (req, res) => {
  try {
    const rows = await dbAll('SELECT * FROM garancia ORDER BY created_at DESC');
    res.json(rows);
  } catch (error) {
    console.error('Hiba a garanciák lekérésekor:', error);
    res.status(500).json({ error: "Adatbázis hiba!" });
  }
});

// Garancia státusz frissítése (admin)
app.put('/api/garancia/:id', async (req, res) => {
  const { id } = req.params;
  const { status } = req.body;
  
  try {
    await dbRun('UPDATE garancia SET status = ? WHERE id = ?', [status, id]);
...
app.post('/api/bejelentes', async (req, res) => {
  const { nev, email, tipus, cim, leiras } = req.body;
  
  if (!nev || !email || !tipus || !cim || !leiras) {
    return res.status(400).json({ error: "Minden mező kitöltése kötelező!" });
  }
  
  try {
    await dbRun(
      `INSERT INTO bejelentesek (nev, email, tipus, cim, leiras, status, created_at) 
       VALUES (?, ?, ?, ?, ?, 'új', datetime('now'))`,
      [nev, email, tipus, cim, leiras]
    );
    
    res.json({ success: true, message: "Bejelentés elküldve!" });
  } catch (error) {
...
// Bejelentések lekérése (admin)
app.get('/api/bejelentesek', async (req, res) => {
  try {
    const rows = await dbAll('SELECT * FROM bejelentesek ORDER BY created_at DESC');
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: "Adatbázis hiba!" });
  }
