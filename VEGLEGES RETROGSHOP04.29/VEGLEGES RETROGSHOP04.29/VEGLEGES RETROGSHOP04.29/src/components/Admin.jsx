...
function Admin() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      alert("Hiba a felhasználók betöltésekor: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchFizetesek = async () => {
    try {
      ...
      alert("Hiba a fizetések betöltésekor: " + err.message);
    }
  };
  ...
  const fetchRendelesek = async () => {
    try {
      ...
      alert("Hiba a rendelések betöltésekor: " + err.message);
    }
    return [];
  };
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    const aktualisRendeles = rendelesek.find(r => r.id === orderId);
    if (aktualisRendeles && aktualisRendeles.statusz === newStatus) {
      alert(`⚠️ A rendelés már ${getStatusText(newStatus)} állapotban van!`);
      return;
    }
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        if (newStatus === "fizetve") {
          const fizetesExists = fizetesek.some(f => f.rendeles_id === orderId);
          
          if (!fizetesExists) {
            await fetch(`http://localhost:4000/api/fizetes/create-from-rendeles`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                rendelesId: orderId,
                osszeg: aktualisRendeles?.vegosszeg || 0,
                fizetesMod: aktualisRendeles?.fizetes_mod || "bankkartya"
              })
            });
            await fetchFizetesek();
          } else {
            await fetch(`http://localhost:4000/api/fizetes/update-status`, {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                rendelesId: orderId,
        ...
        const ujStatuszSzoveg = getStatusText(newStatus);
        alert(`✅ Rendelés #${orderId} státusz sikeresen módosítva: ${ujStatuszSzoveg}`);
        addSystemNotification(`✅ Rendelés #${orderId} státusz módosítva: ${ujStatuszSzoveg}`, "status_change");
      } else {
        alert(`❌ Hiba a rendelés #${orderId} módosításakor: ${data.error || "Ismeretlen hiba"}`);
        addSystemNotification(`❌ Hiba a rendelés #${orderId} módosításakor`, "error");
      }
    } catch (err) {
      alert(`❌ Hálózati hiba: Nem sikerült módosítani a rendelés státuszát!`);
  ...
  const approveOrder = async (orderId) => {
    const aktualisRendeles = rendelesek.find(r => r.id === orderId);
    if (aktualisRendeles && aktualisRendeles.statusz === "fizetve") {
      alert(` A rendelés már jóvá van hagyva (Fizetve állapotban van)!`);
      return;
    }
      ...
      {showNotifications && (
        <div className="notification-panel">
          ...
          <div className="notification-section">
            <div className="section-title"> Rendszer műveletek ({systemNotifications.length})</div>
          ...
          <div className="notification-footer">
            <button className="clear-all-btn" onClick={() => { 
              ...
              alert("📋 Összes értesítés törölve!");
                    ...
                    <td data-label="Műveletek" className="actions-cell">
                      {user.status === 'active' ? (
                        <button className="action-btn btn-ban" onClick={async () => {
                          if (window.confirm(`Biztosan tiltani szeretnéd ${user.username} felhasználót?`)) {
                            try {
                              ...
                              if (res.ok) { 
                                fetchUsers(); 
                                addSystemNotification(`🔨 ${user.username} felhasználó tiltva`, "ban");
                                alert(`✅ ${user.username} felhasználó sikeresen tiltva!`);
                              }
                            } catch (err) {
                              alert("❌ Hiba történt a tiltás során!");
                            }
                          }
                        }}>🔨 Tiltás</button>
                      ) : (
                        <button className="action-btn btn-unban" onClick={async () => {
                          try {
                            ...
                            if (res.ok) { 
                              fetchUsers(); 
                              addSystemNotification(`🔓 ${user.username} felhasználó tiltása feloldva`, "unban");
                              alert(`✅ ${user.username} felhasználó tiltása feloldva!`);
                            }
                          } catch (err) {
                      ...
                      <button className="action-btn btn-delete" onClick={async () => {
                        if (window.confirm(`Biztosan törölni szeretnéd ${user.username} felhasználót? Ez a művelet nem visszavonható!`)) {
                          try {
                            const res = await fetch(`http://localhost:4000/api/admin/users/${user.id}`, { method: "DELETE" });
                            if (res.ok) { 
                              fetchUsers(); 
                              addSystemNotification(`🗑️ ${user.username} felhasználó törölve`, "delete");
                              alert(`✅ ${user.username} felhasználó sikeresen törölve!`);
                            }
                          } catch (err) {
