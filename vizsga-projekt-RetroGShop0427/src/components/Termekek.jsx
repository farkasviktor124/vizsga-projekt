...
function Termekek() {
  ...
  const loadProducts = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        // Automatikusan beállítjuk a max árat a legdrágább termék alapján
  ...
  const filterAndSortProducts = useCallback((products) => {
    ...
    console.log("Szűrés előtt:", filtered.length);
    ...
    console.log("Állapot szűrés után:", filtered.length);

    // Ár szűrés
    filtered = filtered.filter(t => {
      const ar = t.akcios_ar ?? t.ar ?? 0;
      const benneVan = ar >= arMin && ar <= arMax;
      if (!benneVan) {
        console.log(`Kiszűrve ár miatt: ${t.nev} (${ar} Ft, min:${arMin}, max:${arMax})`);
      }
      return benneVan;
    });
    console.log("Ár szűrés után:", filtered.length);
    ...
    console.log("Évjárat szűrés után:", filtered.length);
