// src/components/BarionPayment.jsx
...
const BarionPayment = ({ orderDetails, onSuccess, onCancel }) => {
  ...
  const showSuccessAlert = () => {
    alert(
      "✅ FIZETÉS SIKERES! ✅\n\n" +
      `Rendelés száma: ${rendelesSzam}\n` +
      `Fizetett összeg: ${osszeg.toLocaleString()} Ft\n` +
      `Kártya végződése: **** **** **** ${cardNumber.slice(-4)}\n\n` +
      "A visszaigazolást e-mailben elküldtük.\n" +
      "Köszönjük a vásárlást!"
    );
  };

  // Sikertelen fizetés (megszakítás)
  const showCancelAlert = () => {
    const cancelConfirm = window.confirm(
      "BIZTOSAN MEGSZAKÍTOD A FIZETÉST?\n\n" +
      "Ha megszakítod, a rendelésed nem kerül feldolgozásra!\n\n" +
      "OK - Fizetés megszakítása\n" +
      "Mégse - Vissza a fizetéshez"
    );
    
    if (cancelConfirm) {
      fetch(`${API_BASE_URL}/api/barion/cancel-payment`, {
        method: "POST",
      ...
      }).catch(err => console.error("Megszakítási hiba:", err));
      
      alert("❌ A fizetés megszakításra került!\n\nA rendelésedet nem rögzítettük.");
      if (onCancel) {
        onCancel();
      } else {
        navigate("/kosar");
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setPaymentStep(1);
        alert("❌ A fizetés sikertelen! Kérlek próbáld újra.");
      }
    } catch (error) {
      console.error("Fizetés feldolgozási hiba:", error);
      setError("Hálózati hiba történt! Kérlek próbáld újra.");
      setPaymentStep(1);
      alert("❌ Hálózati hiba! Kérlek ellenőrizd a kapcsolatot és próbáld újra.");
    } finally {
  ...
  return (
    <div className="barion-payment-container">
      <div className="barion-payment-card">
        ...
        <img src={LOGO} alt="RETROGSHOP" className="logo-image" />
          ...
          <div className="barion-body success">
            <div className="success-animation">
              ...
              <div className="order-summary">
                ...
                <p><strong>Kártya:</strong> **** **** **** {cardNumber.slice(-4)}</p>
              </div>
            </div>
            <div className="barion-actions">
